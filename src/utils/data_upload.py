...
@click.option("--data-path", required=True, type=click.Path(exists=True), help="Path to the data")
...
@click.command(name="data-upload", help="upload data to a PVC in the authenticated cluster")
@click.pass_context
def data_upload(
    ctx: click.Context,
    data_path: str,
    ...
    # requires a SVC account in the NS
    # oc create sa my-service-account
    # oc policy add-role-to-user edit system:serviceaccount:default:my-service-account
        ...
        spec=kubernetes.client.V1PodSpec(
            containers=[
                kubernetes.client.V1Container(
                    ...
                    volume_mounts=[kubernetes.client.V1VolumeMount(mount_path="/mnt/storage", name="my-storage")]
                )
            ],
            volumes=[
...
def files_to_pod(local_path, namespace, pod_name, container_path):
    try:
        if os.path.isdir(local_path):
            ...
            exec_cmd = ["oc", "exec", "-i", "-n", namespace, pod_name, "--", "tar", "-xf", "-", "-C", container_path]

            # Open the subprocess for both commands (pipe from tar to oc exec)
            tar_process = subprocess.Popen(tar_cmd, stdout=subprocess.PIPE, stderr=sys.stderr)
            exec_process = subprocess.Popen(exec_cmd, stdin=tar_process.stdout, stderr=sys.stderr)
            ...
            command = ["oc", "cp", local_path, f"{namespace}/{pod_name}:{container_path}"]
